...
module.exports = (function (_super) {
    ...
    Squick.prototype.setupDust = function () {
        ...
        dust.helpers["fetch"] = function (chunk, context, bodies, params) {
            ...
            return chunk.map(function (child) {
                var next = function (i) {
                    if (i == paths.length) {
                        return chunk.end();
                    }
                    _this.getPost(paths[i]).then(function (post) {
                        var data = {};
                        data[key] = post;
    ...
    Squick.prototype.getPost = function (name) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            _this.on("posts-ended", function () { return reject(error); });
        });
    };
    ...
    Squick.prototype.startRender = function (post, template) {
        ...
        stream.once("error", function (err) { return _this.emit("error", err); });
        return stream;
    };
    return Squick;
})(stream_1.Readable);
